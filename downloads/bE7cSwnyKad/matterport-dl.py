...
async def downloadAssets(base, base_page_text):
    ...
    # Try new format first (2025+): l.u=e=>"js/"+({...}[e]||e)+".js" and l.miniCssF=e=>"css/"+({...}[e]||e)+".css"
    # This format no longer has separate hash keys - just named chunks
    js_new_match = re.search(r'\.u\s*=\s*\w+\s*=>\s*"js/"\s*\+\s*\(\s*(\{[^}]+\})', showcase_cont)
    css_new_match = re.search(r'\.miniCssF\s*=\s*\w+\s*=>\s*"css/"\s*\+\s*\(\s*(\{[^}]+\})', showcase_cont)
    
    if js_new_match and css_new_match:
        # New format detected - parse the simple name dictionaries
        def extractSimpleJSDict(dict_str):
            """Extract dictionary from format like {239:"three-examples",777:"split"}"""
            ret = {}
            # Remove braces and split by comma
            inner = dict_str.strip()[1:-1]
            # Match patterns like 239:"three-examples" or 239:'three-examples'
            pairs = re.findall(r'(\d+)\s*:\s*["\']([^"\']+)["\']', inner)
            for num, name in pairs:
                ret[num] = name
            return ret
        
        jsNamedDict = extractSimpleJSDict(js_new_match.group(1))
        cssNamedDict = extractSimpleJSDict(css_new_match.group(1))
        
        # Collect chunk IDs from runtime file first
        chunk_ids = set(re.findall(r'\.e\((\d+)\)', showcase_cont))
        
        # Download showcase.js and scan for chunk IDs
        showcase_js_filename = MAIN_SHOWCASE_FILENAME
        if showcase_js_filename:
            showcase_js_cont = await downloadFileAndGetText("SHOWCASE_JS", True, base + showcase_js_filename, showcase_js_filename, always_download=CLA.getCommandLineArg(CommandLineArg.REFRESH_KEY_FILES))
            chunk_ids.update(re.findall(r'\.e\((\d+)\)', showcase_js_cont))
        
        # In the new format, JS files are [name].js if in dict, otherwise [number].js
        # First add all named ones and download them to scan for more chunks
        added_numbers = set()
        all_scanned_content = showcase_cont + (showcase_js_cont if showcase_js_filename else "")
        
        # Download and scan named JS files
        for number, name in jsNamedDict.items():
            file = f"js/{name}.js"
            typeDict[file] = "SHOWCASE_DISCOVERED_JS"
            assets.append(file)
            added_numbers.add(number)
            # Download and scan this named JS file for chunk references
            try:
                js_content = await downloadFileAndGetText("SHOWCASE_DISCOVERED_JS", True, base + file, file, always_download=False)
                all_scanned_content += js_content
            except Exception:
                pass  # File might not exist
        
        # Now add numbered chunks that aren't in the named dict
        for chunk_id in chunk_ids:
            if chunk_id not in added_numbers:
                file = f"js/{chunk_id}.js"
                typeDict[file] = "SHOWCASE_DISCOVERED_JS"
                assets.append(file)
        
        # CSS files - add named ones and also numbered ones from chunk_ids
        for number, name in cssNamedDict.items():
            file = f"css/{name}.css"
            typeDict[file] = "SHOWCASE_DISCOVERED_CSS"
            assets.append(file)
        
        # IMPORTANT: Do multiple passes to find all chunks recursively
        # Keep scanning downloaded JS files until no new chunks are found
        max_passes = 5  # Limit passes to avoid infinite loops
        
        for pass_num in range(max_passes):
            new_chunks_found = set()
            
            # Find all chunk IDs in all scanned content using multiple patterns:
            # 1. Direct .e(number) calls for dynamic imports
            # 2. Module arrays like [moduleId,chunkId] e.g. [58249,8249] - the second number is the chunk
            # 3. webpackChunk push patterns like [[8249],{...}]
            # 4. CSS chunk dictionaries like {2978:1,5385:1,9774:1} - these chunks have associated CSS
            all_chunk_refs = set(re.findall(r'\.e\((\d+)\)', all_scanned_content))
            
            # Find chunks in module arrays: "name":[moduleId,chunkId]
            module_arrays = re.findall(r'\[(\d+),(\d+)\]', all_scanned_content)
            for module_id, chunk_id in module_arrays:
                all_chunk_refs.add(chunk_id)
            
            # Find chunks in webpackChunk push: .push([[chunkId],{
            webpack_chunks = re.findall(r'\.push\(\[\[(\d+)\]', all_scanned_content)
            for chunk_id in webpack_chunks:
                all_chunk_refs.add(chunk_id)
            
            # Find CSS chunk dictionaries: {number:1,number:1,...} pattern in runtime
            # These indicate which chunks have associated CSS files
            css_chunks_with_css = set()
            css_chunk_dicts = re.findall(r'\{(\d+:\d+(?:,\d+:\d+)*)\}', all_scanned_content)
            for dict_match in css_chunk_dicts:
                pairs = re.findall(r'(\d+):\d+', dict_match)
                for chunk_id in pairs:
                    all_chunk_refs.add(chunk_id)
                    css_chunks_with_css.add(chunk_id)
            
            # Download CSS for chunks that are in the CSS chunk dictionary
            # Do this BEFORE checking if chunks are new, because these chunks might already be known
            # but their CSS files haven't been downloaded yet
            for chunk_id in css_chunks_with_css:
                css_file = f"css/{chunk_id}.css"
                if css_file not in assets:
                    typeDict[css_file] = "SHOWCASE_DISCOVERED_CSS"
                    assets.append(css_file)
                    # Try to download the CSS immediately
                    try:
                        await downloadFile("SHOWCASE_DISCOVERED_CSS", True, base + css_file, css_file, always_download=False)
                    except Exception:
                        pass  # CSS might not exist for this chunk
            
            # Download and scan any JS files we haven't scanned yet
            for chunk_id in all_chunk_refs:
                js_file = f"js/{chunk_id}.js"
                if chunk_id not in added_numbers and js_file not in assets:
                    # This is a new chunk we need to download
                    typeDict[js_file] = "SHOWCASE_DISCOVERED_JS"
                    assets.append(js_file)
                    added_numbers.add(chunk_id)
                    new_chunks_found.add(chunk_id)
                    
                    # Also add CSS for this chunk in case it wasn't in the css dictionary
                    css_file = f"css/{chunk_id}.css"
                    if css_file not in assets:
                        typeDict[css_file] = "SHOWCASE_DISCOVERED_CSS"
                        assets.append(css_file)
                    ...
                    # Download and scan this new JS file for more chunk references
                    try:
                        js_content = await downloadFileAndGetText("SHOWCASE_DISCOVERED_JS", True, base + js_file, js_file, always_download=False)
                        all_scanned_content += js_content
                    except Exception:
                        pass  # File might not exist
            
            # If no new chunks were found in this pass, we're done
            if not new_chunks_found:
                break
    else:
        # Fall back to old format with hash keys
        match = re.search(
            r"""
...
async def downloadPlugins(pageid):
    ...
    for plugin in pluginJson:
        ...
        current_version = plugin.get("currentVersion")
        versions = list((plugin.get("versions") or {}).keys())
        candidate_versions: list[str] = []
        if current_version:
            candidate_versions.append(current_version)
            if "-" in current_version:
                candidate_versions.append(current_version.split("-")[0])
        candidate_versions.extend(versions)
        ...
        for version in candidate_versions:
            if not version or version in tried:
                continue
            tried.add(version)
            plugPath = f"showcase-sdk/plugins/published/{name}/{version}/plugin.json"
            try:
                await downloadFile("PLUGIN", True, f"https://static.{BASE_MATTERPORT_DOMAIN}/{plugPath}", plugPath)
                # If we downloaded a suffixed version (e.g., 1.0.14-1) and the base version
                # (e.g., 1.0.14) is requested by the client, copy to the base path.
                if current_version and version == current_version and "-" in current_version:
                    base_version = current_version.split("-")[0]
                    if base_version and base_version != current_version:
                        base_path = f"showcase-sdk/plugins/published/{name}/{base_version}/plugin.json"
                        if os.path.exists(plugPath) and not os.path.exists(base_path):
                            makeDirs(os.path.dirname(base_path))
                            try:
                                shutil.copyfile(plugPath, base_path)
                            except Exception:
                                pass
                break
            except Exception:
...
class OurSimpleHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parsed_path == "/api/mp/models/graph":
                ...
                return
            elif parsed_path == "/api/mp/accounts/graph":
                # Return empty account data for offline viewing
                self.send_response(200)
                self.send_header("Content-type", "application/json")
                self.end_headers()
                self.wfile.write(bytes('{"data":{"currentUserAccount":null}}', "utf-8"))
                return
        except Exception as error:
            logLevel = logging.ERROR
